...
class TestMCPToolsListAuthSurfacing:
    """Regression: MCP tools/list auth failures must surface as MCPUpstreamAuthError.

    Previously a missing/expired per-user OAuth token, or an upstream 401/403 for
    any non-carveout auth_type, was swallowed to an empty tool list, so a
    single-server client saw a 200 with no tools instead of a 401 challenge. The
    listing helpers now raise MCPUpstreamAuthError for every auth failure; the
    single-server routes turn it into a 401 + WWW-Authenticate while the
    aggregator absorbs it to an empty list.
    """

    @pytest.mark.asyncio
    @pytest.mark.parametrize("status_code", [401, 403])
    async def test_fetch_tools_with_timeout_surfaces_upstream_auth_failure(
        self, status_code
    ):
        ...
        client.list_tools = AsyncMock(
            side_effect=_upstream_status_error(status_code, challenge)
        )
